import type { CHRAttempt, Constraint, Cut, GoalId, Location, R, StepIdx, Time, TimedItem, V, Event } from './types';
import { writeItem } from './trace_v2';
...
export function allChains(key: string, prefixes: string[], l: TimedItem[]): NonEmpty<TimedItem>[] {
...
export function inferChains(key: string, l: TimedItem[]): NonEmpty<TimedItem>[] {
  return allChains(key, ['user:assign', 'user:backchain:fail-to'], l);
}

export function chrEventChains(key: string, l: TimedItem[]): NonEmpty<TimedItem>[] {
...
export function decodeInferAttempt(l: NonEmpty<TimedItem>): { loc: Location, code: string, events: Event[] } {
...
export function decodeCHRTryList(l: NonEmpty<TimedItem>[]): { successful: CHRAttempt[], failed: CHRAttempt[] } {
...
const fileRegex = /File "(?<name>[^"]+)", line (?<line>\d+), column (?<col>\d+), characters (?<beg>\d+)-\d+/;
